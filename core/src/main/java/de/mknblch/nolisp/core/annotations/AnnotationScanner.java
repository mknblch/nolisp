...
public class AnnotationScanner {
    ...
    private static boolean isNonSpecialForm(Method method) throws FunctionDefinitionException {
        ...
        if (
                ...
                !Context.class.equals(types[0])) throw new FunctionDefinitionException(String.format(
                    "Invalid method signature in '%s.%s(..)'. Expected: 'annotations(env:Context, args:ListStruct):Object'",
    ...
    private static boolean isSpecialForm(Method method) throws FunctionDefinitionException {
        ...
        if (3 != types.length ||
            ...
            throw new FunctionDefinitionException(String.format("Invalid signature in METHOD '%s.%s(..)'. Expected: 'annotations(ip:Interpreter, env:Context, args:ListStruct):Object'", method.getDeclaringClass().getSimpleName(), method.getName()));
        }
